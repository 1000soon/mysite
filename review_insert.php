...
$query = "SELECT * FROM tb_board_review WHERE idx=".$num;
...
if(isset($_POST['form1']) && $_POST['form1']=="form1"){
	...
	if($idx>0){
		$query="UPDATE tb_board_review SET v_title=:title, v_name=:name, v_content=:content, v_pass=:pw, ip_addr=:ip WHERE idx=:idx";	
		$stmt = $dbh->prepare($query);
